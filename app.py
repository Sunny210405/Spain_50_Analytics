...
def main() -> None:
    ...
    # Album rail toggle — session state is read each run so no st.rerun() needed
    ...
    if full_artwork_count > 6:
        ...
        with btn_col:
            if is_expanded:
                # Button click writes to session_state and triggers normal Streamlit rerun
                st.button("Show Less ↑", key="toggle_covers_btn", width="stretch",
                          on_click=lambda: st.session_state.update({"show_all_covers": False}))
            else:
                st.button(f"Show All Top {full_artwork_count} ↓", key="toggle_covers_btn", width="stretch",
                          on_click=lambda: st.session_state.update({"show_all_covers": True}))
